		...
		async.eachSeries( changed,
			function(file, callback) {
				...
				fs.stat( file, function(err, stats) {
					...
					// macOS Sonoma+ seems to randomly "change" old files, so we need a mtime sanity check
					if (stats && (stats.mtimeMs < Date.now() - 60000)) {
						self.logDebug(9, "Ignoring change event for old file: " + file, stats);
						return callback();
					}
